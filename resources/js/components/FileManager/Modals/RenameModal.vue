<template>
    ...
    >
          <div class="relative mx-auto z-20 max-w-2xl text-left">
            <form @submit.prevent="$emit('rename')" autocomplete="off" class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
                <div class="space-y-6">
                    <h3 class="uppercase tracking-wide font-bold text-xs border-b border-gray-100 dark:border-gray-700 py-4 px-8">Rinomina Elemento</h3>
                    <div class="px-8">
                        <label for="itemName" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Nuovo Nome</label>
                        <input
                            type="text"
                            id="itemName"
                            :value="modelValue"
...
const emit = defineEmits(['close', 'rename', 'update:modelValue']);
